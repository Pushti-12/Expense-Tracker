...
db_config = {
    ...
    "password": "sumeet",  # Replace with your MySQL password
...
def signup():
    try:
        ...
        # Validate inputs
        if not username or not password or not birthdate or not email:
        ...
        cursor.execute(
            "INSERT INTO users (user_id, username, password, birthdate, email) VALUES (%s, %s, %s, %s, %s)",
            (user_id, username, password, birthdate, email)
        )
        connection.commit()
...
def forgot_password():
    try:
        ...
        # Retrieve form data (username and dob)
        username = request.json.get('username')
        ...
        # Check if the user exists with the given username and dob
        cursor.execute("SELECT * FROM users WHERE username = %s AND birthdate = %s", (username, dob))
        user = cursor.fetchone()

        if user:
            return jsonify({"message": "User verified. You can now reset your password."}), 200
        else:
            return jsonify({"error": "Username or Date of Birth is incorrect!"}), 400
